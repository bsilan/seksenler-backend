...
| 6 | ✅ **Frontend** — TAMAMLANDI (`frontend/`, Vite + React) | Onaylanan '80'ler gazete tasarımı birebir uygulandı: giriş, lobi, basılı-tut rol kartı, oyun tahtası, aday gösterme, oylama, yasama, özel yetkiler, veto ve oyun sonu ekranları. 2 sn'de bir game_state polling; oturum localStorage'da (sayfa yenilenince oyuna dönülür). 5 oyunculu tam tur gerçek tarayıcıda (Playwright) test edildi. |
| 7 | ✅ **Deploy hazırlığı** — TAMAMLANDI (yayınlama kullanıcıda) | Mimari: frontend → Vercel, backend → Render (bellek-içi oyun durumu sürekli açık süreç gerektirdiği için Vercel'de barınamaz). `render.yaml` blueprint hazır; tıklama-tıklama rehber `DEPLOY.md`'de. Yayınlamayı kullanıcı kendi GitHub/Vercel/Render hesaplarıyla yapacak. |
